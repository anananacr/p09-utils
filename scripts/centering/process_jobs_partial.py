...
def exclude_processed_data(files: List[str], label_output_file: str) -> List[str]:
    ...
    if file_exists == 0:
        ...
        df = pd.read_csv(
            label_output_file + "_mean.csv", usecols=["param_value"], dtype=int
        )
...
def initialize_files(label_output_file: str):
    ...
    for i in func:
        ...
        df.to_csv(
            f"{label_output_file}_{i}_std.csv", index=False, mode="w", header=True
        )
...
def process_files(files: List[str], center_file: str, label_output_file: str):
    ...
    for i in func:
        ...
        mean_data.to_csv(
            f"{label_output_file}_{i}.csv", index=False, mode="a", header=False
        )
        std_data.to_csv(
            f"{label_output_file}_{i}_std.csv", index=False, mode="a", header=False
        )
...
def main(raw_args=None):
    ...
    files = search_files_filtered(
        folder_path,
        file_name="gen_images*",
        file_format="h5",
        sort=True,
        label_output_file=label,
    )
